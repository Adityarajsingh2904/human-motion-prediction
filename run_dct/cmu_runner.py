...
def L2NormLoss_test(gt, out, frame_ids):  # (batch size,feature dim, seq len)
    ...
    gt = (
        gt.permute(0, 2, 1).contiguous().view(batch_size, seq_len, -1, 3)
    )  # B, 25, 22, 3
    out = (
        out.permute(0, 2, 1).contiguous().view(batch_size, seq_len, -1, 3)
    )  # B, 25, 22, 3
    for k in np.arange(0, len(frame_ids)):
        ...
        t_3d[k] = (
            torch.mean(
                torch.norm(
                    gt[:, j, :, :].contiguous().view(-1, 3)
                    - out[:, j, :, :].contiguous().view(-1, 3),
                    2,
                    1,
                )
            )
...
class CMURunner:
    def __init__(
        ...
        print("==========================================\n")
        with open(
            os.path.join(self.cfg.ckpt_dir, "config.txt"), "w", encoding="utf-8"
        ) as f:
        ...
        if self.cfg.output_n == 25:
            self.model = MSRGCN(
                self.cfg.p_dropout,
                self.cfg.leaky_c,
                self.cfg.final_out_noden,
                input_feature=self.cfg.dct_n,
            )
        elif self.cfg.output_n == 10:
            self.model = MSRGCNShortTerm(
                self.cfg.p_dropout,
                self.cfg.leaky_c,
                self.cfg.final_out_noden,
                input_feature=self.cfg.dct_n,
        ...
        train_dataset = CMUMotionDataset(
            self.cfg.base_data_dir,
            actions="all",
            mode_name="train",
            input_n=self.cfg.input_n,
            output_n=self.cfg.output_n,
            dct_used=self.cfg.dct_n,
            split=0,
            sample_rate=2,
            down_key=[
                ("p22", "p12", self.cfg.Index2212),
                ("p12", "p7", self.cfg.Index127),
                ("p7", "p4", self.cfg.Index74),
            ],
            test_manner=self.cfg.test_manner,
            global_max=0,
            global_min=0,
        ...
        for act in define_actions_cmu("all"):
            ...
            print(
                ">>> test {} data {}".format(
                    act, test_dataset.gt_all_scales["p32"].shape[0]
                )
            )
    ...
    def restore(self, checkpoint_path):
        ...
        print(
            "load from lr {}, curr_avg {}, best_avg {}.".format(
                state["lr"], curr_err, best_err
            )
        )
    ...
    def train(self, epoch):
        ...
        for i, (inputs, gts) in tqdm(
            ...
            for k in inputs:
                inputs[k] = (
                    inputs[k].float().cuda(non_blocking=True, device=self.cfg.device)
                )
            ...
            for k in outputs:
                ...
                outputs[k] = (
                    outputs[k] * (self.global_max - self.global_min) + self.global_min
                )

                outputs[k] = reverse_dct_torch(
                    outputs[k], self.i_dct_m, self.cfg.seq_len
                )
        ...
        for act_idx, act in enumerate(define_actions_cmu("all")):
            ...
            for i, (inputs, gts) in enumerate(self.test_loader[act]):
                ...
                for k in inputs:
                    inputs[k] = (
                        inputs[k]
                        .float()
                        .cuda(non_blocking=True, device=self.cfg.device)
                    )
                    gts[k] = (
                        gts[k].float().cuda(non_blocking=True, device=self.cfg.device)
                    )
                with torch.no_grad():
                    outputs = self.model(inputs)
                    for k in outputs:
                        outputs[k] = (outputs[k] + 1) / 2
                        outputs[k] = (
                            outputs[k] * (self.global_max - self.global_min)
                            + self.global_min
                        )
                    ...
                    mygt = (
                        gts["p32"]
                        .view(-1, self.cfg.origin_noden, 3, self.cfg.seq_len)
                        .clone()
                    )
                    myout = outputs["p22"].view(
                        -1, self.cfg.final_out_noden, 3, self.cfg.seq_len
                    )
                    mygt[:, self.cfg.dim_used_3d, :, :] = myout
                    mygt[:, self.cfg.dim_repeat_32, :, :] = myout[
                        :, self.cfg.dim_repeat_22, :, :
                    ]
                    mygt = mygt.view(-1, self.cfg.origin_noden * 3, self.cfg.seq_len)

                    loss = L2NormLoss_test(
                        gts["p32"][:, :, self.cfg.input_n :],
                        mygt[:, :, self.cfg.input_n :],
                        self.cfg.frame_ids,
                    )
                    ...
                    if act_idx == 0 and i == 0:
                        pred_seq = (
                            ...
                            .cpu()
                            .data.numpy()[0]
                            .reshape(self.cfg.final_out_noden, 3, self.cfg.seq_len)
                        )
                        gt_seq = (
                            gts["p22"]
                            .cpu()
            ...
            for fidx, frame in enumerate(frame_ids):
                self.summary.add_scalar(
                    f"Test/{act}/{frame}", total_loss[act_idx][fidx], epoch
                )

        self.summary.add_scalar("Test/average", np.mean(total_loss), epoch)
        for fidx, frame in enumerate(frame_ids):
            self.summary.add_scalar(
                f"Test/avg{frame}", np.mean(total_loss[:, fidx]), epoch
            )
        return total_loss
    ...
    def run(self):
        for epoch in range(self.start_epoch, self.cfg.n_epoch + 1):
            ...
            if average_train_loss < self.best_accuracy:
                ...
                self.save(
                    os.path.join(
                        self.cfg.ckpt_dir,
                        "models",
                        "{}_in{}out{}dctn{}_best_epoch{}_err{:.4f}.pth".format(
                            self.cfg.exp_name,
                            self.cfg.input_n,
                            self.cfg.output_n,
                            self.cfg.dct_n,
            ...
            if epoch % 1 == 0:
                ...
                print(
                    "Epoch: {},  LR: {}, Current err test avg: {}".format(
                        epoch, self.lr, np.mean(loss_l2_test)
                    )
                )
